...
def train_model(train_loader, model, criterion, optimizer, epoch, tb_writer, study_type):
    ...
    for i, data in enumerate(pbar):
        ...
        weights = [LOSS_WEIGHTS[labels[i]][study_type] for i in range(inputs.size(0))]
...
def valid_model(valid_loader, model, criterion, optimizer, epoch, tb_writer, study_type=None):
    losses = AverageMeter('epoch_loss')
    accs = AverageMeter('epoch_acc')
    # ensure model is in train mode
...
def main():
    ...
    parser.add_argument('--study_type', default=None, type=str, required=True)
    ...
    train_loader = get_dataloaders('train', batch_size=args.batch_size, shuffle=True, type=args.study_type)

    valid_loader = get_dataloaders('valid', batch_size=args.batch_size, shuffle=False, type=args.study_type)
    ...
    for e in range(args.epochs):
        train_out = train_model(train_loader, sess.net,
                                criterion, optimizer, clock.epoch, tb_writer, study_type=args.study_type)
